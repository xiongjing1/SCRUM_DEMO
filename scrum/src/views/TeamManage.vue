<template>
  ...
  <div class="main">
    <div class="title">
      <div class="team">
        <div class="TeamPhoto">
          <img :src="this.teamshot" style="height: 60px;width:60px;background-color:cornflowerblue;margin-top: 10px;float: left;margin-left: 20px;border-radius: 100%">
        </div>
        <div class="TeamName">
          {{ team.t_name }}
        </div>
...
export default {
  ...
  mounted() {
    ...
    this.$axios.get('http://43.138.21.64:8080/user/'+window.localStorage.getItem('uid')+'/team/'+window.localStorage.getItem('tid')).then((res) => {
      ...
      this.teamshot='http://43.138.21.64:8080/'+this.team.t_headshot
      this.load();
